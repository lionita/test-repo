...
### 4) Missing platform components from design
...
### 5) Consistency/retry strategy gap
...
### 6) Security/compliance/observability gaps
...
2. Replace in-process event publisher with external broker integration if cross-service consumers are required.
3. Add observability baseline (metrics + tracing + correlation IDs) before scaling.
